...
if [ "$cygwin" = "true" ] || [ "$msys" = "true" ] ; then
    ...
    # Now add the required pattern matching for the cygpath command
    OURCYGPATTERN="$OURCYGPATTERN|(^.*;)"
    if [ "$OURCYGPATTERN" != "" ]; then
        # Now convert the arguments - kludge to limit ourselves to /bin/sh
        for arg in "$@"; do
            CHECK=`echo "$arg"|egrep -c "$OURCYGPATTERN" -`
            CHECK2=`echo "$arg"|egrep -c "^-"`                      # Check if argument is an option
...
# by default we should be in the "presentation" directory
cd "$APP_HOME"
